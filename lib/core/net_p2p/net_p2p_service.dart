// lib/core/net_p2p/net_p2p_service.dart
//
// NetP2P 服务 — v2 pub/sub 模式（不再依赖 DataLog/scope）

import 'dart:async';

import 'package:xiaodouzi_fr/core/net_engine/net_engine.dart' as fw;

import 'net_p2p_message.dart';
    ...
    _sub = t.subscribe('room/$code/events').listen((ev) {
      ...
      if (p['type'] == 'peer-joined' || p['type'] == 'peer-online') {
        ...
        _eventsCtrl.add(MessageReceived(
          NetP2PMessage.fromTransportEvent(p),
        ));
      }
    });
  }
...
final netP2PService = NetP2PService();
