...
>
> **v6.2 起（2026-08-29 追加，2026-09-03 迁移）**：新增 **KV public 皮肤 meta 覆盖** —— dart 端 7 套 catalog 作基线，
> 每次进入换肤设置页时按需匿名拉一次 `GET /api/v1/kv/public/chess_skin:index?groupId=190`，成功则覆盖/扩展注册表
...
- `fetchAndMergeSkins()`（ChessSkinSettingsPage._fetchKvSkins 里 `unawaited` 语义，5s 超时 best-effort）：成功 → 覆盖/扩展注册表并 setState 闪增；
...
```
写完后客户端下次进入换肤页即生效；更新同一 key 即可免发版换肤。
...
## 5. 何时调 `ChessSkinBundle.registerHardcoded()` / `fetchAndMergeSkins()`

- `registerHardcoded()`：启动期调用一次（`lib/main.dart` 在 `runApp()` 之前调，仅装本地 7 套）。
- `fetchAndMergeSkins()`：**不在启动期调**（2026-09-03 迁移）；改为每次进入换肤设置页时按需调（`ChessSkinSettingsPage.initState` → `_fetchKvSkins()`，后台 5s 超时，成功后 setState 追加）。

```dart
// lib/main.dart
void main() {
  WidgetsFlutterBinding.ensureInitialized();
  ChessSkinBundle.registerHardcoded();  // ← 仅本地 catalog，零网络
  runApp(...);
}
