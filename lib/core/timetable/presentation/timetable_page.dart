...
import 'cell_actions/cell_action_manager.dart';
...
class _TimetablePageState extends ConsumerState<TimetablePage> {
  ...
  /// cell 管理器（策略模式，fr 28）：添加/编辑 cell 按模式分派，
  /// 页面不感知课表/追剧的模式分支
  final CellActionManager _cellActions = CellActionManager();

  /// 打开编辑器（添加/编辑 cell 的统一触发入口）
  void _openEditor(
    int cycleIndex,
    int dayOfCycle,
    int slotIndex,
    ...
    _cellActions.openEditor(
      CellActionContext(
        context: context,
        ref: ref,
        onSelectionChanged: (key) => setState(() => _selectedCellKey = key),
      ),
      CellTarget(
        cycleIndex: cycleIndex,
        dayOfCycle: dayOfCycle,
      ...
      ref.read(TimetableStore.configProvider),
    );
  }
}
