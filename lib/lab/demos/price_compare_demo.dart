...
class _PriceComparePageState extends State<_PriceComparePage> {
  ...
  Future<void> _init() async {
    ...
    // 恢复上次主题；没有就新建
    final lastId = await PriceCompareRepository.instance.getLastTopicId();
    PriceTopic? loaded;
    if (lastId != null) {
      loaded = await PriceCompareRepository.instance.getTopic(lastId);
    }
  ...
  /// 幂等：已注册则跳过。Hive 操作委托给 PriceCompareRepository，避免 demo 直接 import hive。
  void _registerToStorageRegistry() {
    ...
    StorageRegistry.register(BoxDescriptor(
      ...
      displayName: '比价主题',
      openUntyped: PriceCompareRepository.openBoxForDescriptor,
      formatValue: (v) {
        if (v is Map) {
          final title = v['title'];
          final rows = v['rows'];
  ...
  Future<PriceTopic> _createNewTopic({bool persist = false}) async {
    final id = persist
        ? await PriceCompareRepository.instance.createEmptyTopic()
        : 't${DateTime.now().microsecondsSinceEpoch}';
    final t = PriceTopic(id: id);
    if (persist) {
      await PriceCompareRepository.instance.setLastTopicId(id);
    }
    return t;
  }
  ...
  Future<void> _persistNow() async {
    final t = _topic;
    if (t == null) return;
    await PriceCompareRepository.instance.putTopic(t);
    await PriceCompareRepository.instance.setLastTopicId(t.id);
  }
  ...
  Future<void> _switchTopic() async {
    await _persistNow();
    final summaries = await PriceCompareRepository.instance.listSummaries();
    if (!mounted) return;
    ...
    if (selected.startsWith('__deleted__:')) {
      ...
      if (deletedId == _topic?.id) {
        // 当前被删；选下一个或新建
        final remaining = await PriceCompareRepository.instance.listSummaries();
        final t = remaining.isNotEmpty
            ? await PriceCompareRepository.instance.getTopic(remaining.first.id)
    ...
    final loaded = await PriceCompareRepository.instance.getTopic(selected);
    if (loaded != null) {
