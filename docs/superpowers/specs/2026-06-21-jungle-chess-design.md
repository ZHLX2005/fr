...
机械照搬 `lib/core/surround_game/` 的 LAN 实现模式，补完 9 个文件(4 新增 + 5 改写)：

- **新增**: `lan/persistence/device_id_service.dart`、`lan/persistence/player_profile_service.dart`、
  `lan/lan_room_page.dart`、`widgets/jungle_host_touch_controller.dart`(含两个工厂)
- **改写**: `lan/serializer/game_state_serializer.dart`(加 `implements StateSerializer`)、
  `lan/service/lan_service_adapter.dart`(从 TODO 替换为 280 行完整实现)、
  `lan/lan_lobby_page.dart`(从 66 行占位扩到 ~280 行)、
  `lan/lan_host_game_page.dart`(加 Session + notifier 桥接)、
  `lan/lan_client_game_page.dart`(严格只读文字版)

关键约束：
- Host-Authoritative 模型：Host 唯一执行引擎，Client 只接收 state
- 两段式 Session：房间 UDP 多播 + 游戏 HTTP Session
- Client 严格只读，不渲染棋盘，不响应触摸
- 倒计时为页面本地驱动(不修改 sealed event/state 文件)
