...
> **适用边界（先读）**：本方案只适用于**功能型**界面——工具页、表单、设置、密集操作按钮区。
> **内容型**页面（游戏中心 / 媒体库 / 作品集）反向操作：封面、Hero 头部、主 CTA 该填充就填充，
> 每条内容还要有专属识别色，见 [[store-style-content-page]]。把本方案当全局禁令套上去，
> 结果就是所有卡片长一样的"demo 形态"。
...
| 元素类型                                  | 配色策略                                                       | 例子                     |
| ----------------------------------------- | -------------------------------------------------------------- | ------------------------ |
| **导航 / 功能入口**（菜单卡 icon、tab、纯装饰） | **统一主题色** `Theme.of(context).colorScheme.primary` | profile 菜单卡片         |
| **功能性**（操作按钮）              | **撞色编码语义**                                         | APK / KV / 文件 tab 按钮 |
| **内容条目**（游戏、媒体、作品、模板） | **每条专属识别色**，集中登记在 `const_xxx.dart` 的 slug 表 | 游戏中心卡片封面 → [[store-style-content-page]] |
...
| 错误操作                                                | 实际后果                            | 正确做法                                                   |
| ------------------------------------------------------- | ----------------------------------- | ---------------------------------------------------------- |
| 给**导航/功能入口**配彩虹独立色                        | 显幼稚、违和、不统一                | 导航 icon 用统一主题色（**内容条目不适用**：作品卡就该有专属色）                                     |
...
| 在**同一片密集按钮区**里留个别纯色填充 CTA                            | 用户仍投诉"还有纯色"                | 该区域内全转 OutlinedButton；层级靠**颜色 + 位置**。⚠️ 仅限本区域——不要外推到内容页封面 / Hero 头部 |
