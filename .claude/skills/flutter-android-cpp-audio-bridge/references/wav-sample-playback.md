...
description: 在 Oboe audio callback 里混入 WAV 采样替代合成音色 — dr_wav.h 加载、slot 管理、resample、FFI 传路径。读完 SKILL.md 的核心架构 + 6 步 SOP 之后、需要自定义拍声音色时加载。~
---
...
**前提**：你已经有一个跑通的 Oboe bridge（SKILL.md 的 6 步 SOP 已经做完）。本 ref 是在既有 `onAudioReady` 回调里叠加采样层。
...
```
C++ 侧                       Dart 侧
gSlots[3]                    SampleLoader.materializeAsset()
  ├─ pcm: float*               → rootBundle.load → 写文件
  ├─ lengthSamples: int        → 返回绝对路径
  └─ playPos: int              ↓
       (-1 = idle)          MetronomeFFI.loadSample(level, path)
                                → load_sample(int level, const char* path)
                                → dr_wav 解码 + downmix + resample
                                → 存入 gSlots[level]
```
...
MetronomeFFI.loadSample(level, path);  // level: 0=weak, 1=medium, 2=accent
```
...
| Dart FFI | `ffi_bindings.dart` | `loadSample` / `clearSample` |
| Dart asset | `sample_loader.dart` | asset → 文件系统拷贝 |
| Dart controller | `metronome_controller.dart` | `setSoundForLevel` per-level API |
| Dart UI | `metronome_demo.dart` | 音色选择器（带 ui→cpp slot 映射） |
...
| 做法 | 结果 |
|------|------|
| 在 _ensureReady 里 fire-and-forget 预加载 | 无感知，首个 beat 靠合成 fallback，后续自动切到采样 |
